...
%this code is associated with, the all nonzero entries of the parameter vector s are constant.

%load the data set
load("{PLACEHOLDER.mat"); %Change this to the other sets of data provided or your own set.
...
function [eigs_i] = eigsApprox(p_i,q_i,n,guess)
    ...
    for k = 1:c
        ...
        if guess == -100*ones(c,1)
            t0 = (a_k + b_k)/2;
            iter2 = 750; %if this is the first guess take 750 steps
        else
            t0 = guess(k);
            iter2 = 50; %if this is a seeded guess only take 50 steps
        end
        for j = 1:iter2
