...
load("C:\Users\fergu\Google Drive\Daniel\regression\Rk_to_graph\Approximate_Frechet_Mean_Data\regression_data.mat");
...
stepSize = 0.01;%number of times to estimate approximate empirical Frechet Regression
...
for time = 0:stepSize:1
    ...
    maxSteps = 100;
    curStep = 1;
    h = 0.0001; %Stepsize for linesearch/gradient descent
                %Having a variable stepsize would be beneficial. Large steps early and smaller steps as the algorithm converges
    ...
    while(norm(p_i-p_prev)/norm(p_prev) > 1*10^-5 && curStep < maxSteps) %Various different stopping condition can be used
        p_prev = p_i;
